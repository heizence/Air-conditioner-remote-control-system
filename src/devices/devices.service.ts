...
export class DevicesService {
  ...
  async pollCommands(deviceId: string) {
    // 1. 디바이스의 마지막 통신 시간(lastPingAt) 업데이트
    // DB에서 해당 deviceId를 가진 디바이스를 찾습니다.
    ...
    if (deviceIndex === -1) {
      ...
      // 디바이스가 존재하면, 상태와 마지막 통신 시간을 업데이트합니다.
    ...
    const commandToProcess = await this.dbService.db.find<Command>(
      '/commands',
      // deviceId가 일치하고 status가 'pending'인 첫 번째 명령을 찾습니다.
    ...
    if (commandToProcess) {
      ...
      // 명령의 상태를 'in-progress'(처리 중)으로 변경합니다.
      await this.dbService.db.push(
    ...
    // 처리할 명령을 반환합니다. (없으면 undefined가 반환됨)
